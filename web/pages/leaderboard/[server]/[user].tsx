    ...
    constructor(props: PropsUsers) {
        ...
        this.state = {
            ...
            odometerPointsNeededToNextLevel:
                props.odometerPointsNeededToNextLevel,
            odometerPointsNeededForNextLevel:
                props.odometerPointsNeededForNextLevel,
            odometerProgressToNextLevelPercentage:
                props.odometerProgressToNextLevelPercentage,
            chartOptions: {
                chart: {
                ...
                .then((data) => {
                    ...
                    this.setState((prevState) => {
                        ...
                        return {
                            ...
                            odometerPointsNeededToNextLevel:
                                data.xp_needed_next_level,
                            odometerPointsNeededForNextLevel:
                                points + data.xp_needed_next_level,
                            odometerProgressToNextLevelPercentage:
                                data.progress_next_level,
                    ...
                    <div className="grid grid-cols-4 gap-4 w-full">
                        <div className="bg-gray-800 p-2 rounded-lg flex flex-col justify-center items-center transition-transform transform hover:scale-105">
                            <div className="text-xl sm:text-xl md:text-2xl lg:text-2xl">
                                <Odometer
                                    value={odometerPointsNeededToNextLevel}
                                />
                            </div>
                        ...
                        <div className="bg-gray-800 p-2 rounded-lg flex flex-col justify-center items-center transition-transform transform hover:scale-105">
                            <div className="text-xl sm:text-xl md:text-2xl lg:text-2xl">
                                <Odometer
                                    value={odometerPointsNeededForNextLevel}
                                />
                            </div>
                            <div className="text-gray-400 mt-2 center-text">
...
export async function getServerSideProps(context: {
    query: { server: string; user: string };
}) {
    const { server, user } = context.query;

    try {
        const response = await fetch(
            `http://localhost:18103/get/${server}/${user}`
        );
        ...
        if (response.ok) {
            ...
            return {
                props: {
                    ...
                    odometerPointsNeededForNextLevel: Number(
                        data.xp + data.xp_needed_next_level
                    ),
                    odometerProgressToNextLevelPercentage:
                        data.progress_next_level,
                },
