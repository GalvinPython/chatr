...
const commands: Record<string, Command> = {
    ...
    xp: {
        ...
        execute: async (interaction) => {
            ...
            const optionUser = interaction.options.get("user")?.value as
                | string
                | null;
            const member = (
                optionUser
                    ? interaction.guild!.members.cache.get(optionUser)
                    : interaction.member
            ) as GuildMember;
            ...
            const rank =
                leaderboard.leaderboard.findIndex(
                    (entry: { id: string }) => entry.id === user
                ) + 1;

            const card = new RankCardBuilder()
                .setDisplayName(member.displayName)
                .setAvatar(
                    member.displayAvatarURL({ forceStatic: true, size: 4096 })
                ) // user avatar
                ...
                .setBackground(
                    member.user.bannerURL({ forceStatic: true, size: 4096 }) ??
                        "#23272a"
                );
            ...
            collector.on("collect", async (i) => {
                if (i.user.id !== user)
                    return i.reply({
                        content:
                            "You're not the one who initialized this message! Try running /xp on your own.",
                        ephemeral: true,
                    });
        ...
        data: {
            options: [
                ...
                },
                {
                    name: "role",
                    description:
                        "Enter the role name. Required for add and remove actions.",
                    type: 8,
                    required: false,
                },
                {
            ...
            switch (action) {
                case "get":
                    ...
                    if (Object.keys(roles).length === 0) {
                        await interaction.reply({
                            ephemeral: true,
                            content:
                                "No roles found! This was either an error from the API or you have none!",
                        });
                ...
                case "add":
                    if (!role || !level) {
                        await interaction.reply({
                            ephemeral: true,
                            content:
                                "ERROR: One of these two values were not specified! [role, level]",
                        });

                        return;
                    }
                    apiSuccess = await addRole(
                    ...
                    if (apiSuccess) {
                        await interaction.reply({
                            ephemeral: true,
                            content: `Successfully added <@&${role}> to level ${level}`,
                        });
                    ...
                    await interaction.reply({
                        ephemeral: true,
                        content: `ERROR: Couldn't add <@&${role}> to level ${level}`,
                    });
                ...
                default:
                    if (!role) {
                        await interaction.reply({
                            ephemeral: true,
                            content: "ERROR: Role was not specified!",
                        });
                    }
                    apiSuccess = await removeRole(
                        interaction.guildId as string,
                        role as string
                    );
                    if (apiSuccess) {
                    ...
                    await interaction.reply({
                        ephemeral: true,
                        content: `ERROR: Couldn't remove <@&${role}>`,
                    });
        ...
        data: {
            options: [
                ...
                },
                {
                    name: "channel",
                    description:
                        "Enter the channel ID. Required for set action.",
                    type: 7,
                    required: false,
                },
            ],
            ...
            switch (action) {
                case "enable":
                    ...
                    if (!success) {
                        await interaction
                            .reply({
                                ephemeral: true,
                                content:
                                    "Error enabling updates for this server",
                            })
                            .catch(console.error);
                    ...
                    await interaction
                        .reply({
                            ephemeral: true,
                            content: `Updates are now enabled for this server`,
                        })
                        .catch(console.error);

                    return;
                case "disable":
                    success = await disableUpdates(
                        interaction.guildId as string
                    );
                    if (!success) {
                        await interaction
                            .reply({
                                ephemeral: true,
                                content:
                                    "Error disabling updates for this server",
                            })
                            .catch(console.error);
                    ...
                    await interaction
                        .reply({
                            ephemeral: true,
                            content: "Updates are now disabled for this server",
                        })
                        .catch(console.error);
                ...
                case "set":
                    if (!channelId) {
                        await interaction.reply({
                            ephemeral: true,
                            content: "ERROR: Channel was not specified!",
                        });

                        return;
                    }
                    success = await setUpdatesChannel(
                        interaction.guildId as string,
                        channelId
                    );
                    if (!success) {
                        await interaction
                            .reply({
                                ephemeral: true,
                                content:
                                    "Error setting updates channel for this server",
                            })
                            .catch(console.error);
                    ...
                    await interaction
                        .reply({
                            ephemeral: true,
                            content: `Updates channel has been set to <#${channelId}>`,
                        })
                        .catch(console.error);
                ...
                case "reset":
                    ...
                    if (!success) {
                        await interaction
                            .reply({
                                ephemeral: true,
                                content:
                                    "Error resetting updates channel for this server",
                            })
                            .catch(console.error);
                    ...
                    await interaction
                        .reply({
                            ephemeral: true,
                            content: `Updates channel has been reset to default`,
                        })
                        .catch(console.error);

                    return;
                default:
                    data = await getUpdatesChannel(
                        interaction.guildId as string
                    );
    ...
    cooldown: {
        data: {
            options: [
                ...
                },
                {
                    name: "cooldown",
                    description:
                        "Enter the cooldown in seconds. Required for set action.",
                    type: 4,
                    required: false,
                    choices: [],
                },
        ...
        execute: async (interaction) => {
            ...
            switch (action) {
                case "get":
                    cooldownData = await getCooldown(
                        interaction.guildId as string
                    );
                    if (!cooldownData) {
                        await interaction.reply({
                            ephemeral: true,
                            content: "Error fetching cooldown data!",
                        });
                    ...
                    await interaction.reply({
                        ephemeral: true,
                        content: `Cooldown: ${(cooldownData?.cooldown ?? 30_000) / 1000} seconds`,
                    });
                ...
                case "set":
                    if (!cooldown) {
                        await interaction.reply({
                            ephemeral: true,
                            content: "ERROR: Cooldown was not specified!",
                        });

                        return;
                    }
                    apiSuccess = await setCooldown(
                        interaction.guildId as string,
                        parseInt(cooldown as string) * 1000
                    );
                    if (!apiSuccess) {
                        await interaction.reply({
                            ephemeral: true,
                            content: "Error setting cooldown!",
                        });
                    ...
                    await interaction.reply({
                        ephemeral: true,
                        content: `Cooldown set to ${cooldown} seconds`,
                    });
            ...
            options: [
                {
                    name: "user",
                    description:
                        "The user you want to update the XP or level of.",
                    type: 6,
                    required: true,
                },
                {
        ...
        execute: async (interaction) => {
            ...
            switch (type) {
                case "xp":
                    ...
                    await interaction.reply({
                        ephemeral: true,
                        content: `XP set to ${value} for <@${user}>`,
                    });

                    return;
                case "level":
                    apiSuccess = await setLevel(
                        interaction.guildId as string,
                        user,
                        parseInt(value as string)
                    );
                    ...
                    await interaction.reply({
                        ephemeral: true,
                        content: `Level set to ${value} for <@${user}>`,
                    });
    ...
    sync: {
        ...
        execute: async (interaction) => {
            ...
            await interaction.reply({
                ephemeral: true,
                content: `Syncing data from ${formattedBotNames[bot as keyof typeof formattedBotNames]}...`,
            });
            const apiSuccess = await syncFromBot(
                interaction.guildId as string,
                bot as string
            );
            ...
            if (!apiSuccess) {
                await interaction.editReply({
                    content: `Error syncing data! This might mean that ${formattedBotNames[bot as keyof typeof formattedBotNames]} is not set up for this server, or the leaderboard for this server is not public.`,
                });
