...
export async function getGuild(
    guildId: string
): Promise<[QueryError, null] | [null, Guild | null]> {
    return new Promise((resolve, reject) => {
        pool.query(
            "SELECT * FROM guilds WHERE id = ? AND is_in_guild = ?",
            [guildId, true],
            (err, results) => {
                if (err) {
                    reject([err, null]);
...
export async function updateGuild(
    guild: Omit<Guild, "cooldown" | "updates_enabled" | "updates_channel_id">
): Promise<[QueryError | null, null] | [null, Guild[]]> {
    return new Promise((resolve, reject) => {
        pool.query(
            `
			...
			`,
            [guild.id, guild.name, guild.icon, guild.members, true],
            (err, results) => {
                if (err) {
                    reject([err, null]);
                } else {
...
export async function removeGuild(
    guildId: string
): Promise<[QueryError, null] | [null, true]> {
    return new Promise((resolve, reject) => {
        pool.query(
            "UPDATE guilds SET is_in_guild = ? WHERE id = ?",
            [false, guildId],
            (err) => {
                if (err) {
                    reject([err, null]);
...
export async function setCooldown(
    guildId: string,
    cooldown: number
): Promise<[QueryError, null] | [null, Guild]> {
    return new Promise((resolve, reject) => {
        pool.query(
            "UPDATE guilds SET cooldown = ? WHERE id = ?",
            [cooldown, guildId],
            (err, results) => {
                if (err) {
                    reject([err, null]);
...
export async function getBotInfo(): Promise<
    [QueryError | null, BotInfo | null]
> {
    return new Promise((resolve, reject) => {
        pool.query(
            "SELECT COUNT(*) AS total_guilds, SUM(members) AS total_members FROM guilds",
            (err, results) => {
                if (err) {
                    reject([err, null]);
                } else {
                    const botInfo: BotInfo = {
                        ...
                        total_members:
                            (results as BotInfo[])[0].total_members ?? 0,
                    };
