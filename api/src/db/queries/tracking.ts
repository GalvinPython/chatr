...
export async function addUserToTrackingData(
    userId: string,
    guildId: string
): Promise<void> {
...
export async function getUsersTrackingData(
    userId: string,
    guildId: string
): Promise<[QueryError | null, any]> {
...
export async function getGuildTrackingData(
    guildId: string,
    override: number | null
): Promise<[QueryError | null, null] | [null, any]> {
    const topNumber: number = override || 10;

    return new Promise((resolve, reject) => {
        pool.query(
            "SELECT * FROM tracking WHERE guild_id = ? ORDER BY xp DESC, time ASC LIMIT ?",
            [guildId, topNumber],
            (err, results) => {
                if (err) {
                    reject([err, null]);
...
async function insertUserDataToTracking(
    userId: string,
    guildId: string,
    xp: number
): Promise<[QueryError | null, null]> {
    const time = timestamp;

    return new Promise((resolve, reject) => {
        pool.query(
            "INSERT INTO tracking (user_id, guild_id, xp, time) VALUES (?, ?, ?, ?)",
            [userId, guildId, xp, time],
            (err) => {
                if (err) {
                    reject([err, null]);
                } else {
