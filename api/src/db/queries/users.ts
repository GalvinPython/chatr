...
export async function getUsers(
    guildId: string
): Promise<[QueryError, null] | [null, User[]]> {
    return new Promise((resolve, reject) => {
        pool.query(
            "SELECT * FROM users WHERE guild_id = ? AND user_is_in_guild = ? ORDER BY xp DESC",
            [guildId, true],
            (err, results) => {
                if (err) {
                    reject([err, null]);
...
export async function getUser(
    userId: string,
    guildId: string
): Promise<[QueryError, null] | [null, User | null]> {
    return new Promise((resolve, reject) => {
        pool.query(
...
export async function setXP(
    ...
    return new Promise((resolve, reject) => {
        pool.query(
            "UPDATE users SET xp = ?, level = ?, xp_needed_next_level = ?, progress_next_level = ?, user_is_in_guild = ? WHERE id = ? AND guild_id = ?",
            [
                xp,
                newLevel,
                xpNeededForNextLevel.toFixed(2),
...
export async function setLevel(
    ...
    return new Promise((resolve, reject) => {
        pool.query(
            "UPDATE users SET xp = ?, level = ?, xp_needed_next_level = ?, progress_next_level = ? WHERE id = ? AND guild_id = ?",
            [
                newXp,
                level,
                xpNeededForNextLevel.toFixed(2),
